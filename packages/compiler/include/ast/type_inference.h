...
namespace stride::ast
{
    ...
    std::unique_ptr<IAstType> infer_array_member_type(SymbolTable* symbol_table, const AstArray* array);

    /// Infers the result type of a unary operation
    std::unique_ptr<IAstType> infer_unary_op_type(SymbolTable* symbol_table, const AstUnaryOp* operation);

    /// Infers the result type of a binary operation
    std::unique_ptr<IAstType> infer_binary_op_type(SymbolTable* symbol_table, IBinaryOp* operation);
    ...
    std::unique_ptr<IAstType> infer_identifier_type(const SymbolTable* symbol_table, AstIdentifier* identifier);
