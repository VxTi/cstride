...
{
    const auto function_definition = this->get_function_definition(symbol_table);
    llvm::Function* function = function_definition->get_llvm_function();
    ...
    if (!function)
    {
        throw stride_error(
            ...
            std::format(
                "No symbol registered for function '{}'",
    ...
    llvm::Value* function_body_value = this->_body->codegen(symbol_table, module, builder);
